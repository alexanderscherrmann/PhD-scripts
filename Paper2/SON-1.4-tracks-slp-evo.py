...
#    if float(sim[-8:-5])!=float(amp):
    if not amp in sim:# and sim!='SON-clim':
         continue
    print(sim)
    print(MEDIDS[simid])
    print(ATIDS[simid])
...
#        col='dodgerblue'
 ...
 counter = 0
 ref = ds(dwrf + 'SON-clim/wrfout_d01_2000-12-01_00:00:00')
...
#     if float(sim[-8:-5])!=float(amp):
 ...
 fig.savefig(pappath + 'SON-%s-tracks-Med-slp.png'%amp,dpi=300,bbox_inches='tight')
 plt.close('all')
