...
for simid,sim in enumerate(SIMS):
    ...
    for col,i in zip(['navy','red'],[1,2]):
        ...
        ax.scatter(tlon[loc[loco]],tlat[loc[loco]],color=col,marker='*',s=50)
    ax.set_xlim(-80,60)
    ax.set_ylim(25,80)
    tex=ax.text(0.03,0.93,'(a)',transform=ax.transAxes)
    tex.set_bbox(dict(facecolor='white',edgecolor='white'))

    figg.savefig(pappath + 'DJF-1.4-AT-MED-cyclone-tracks.png',dpi=300,bbox_inches='tight')
    plt.close(figg)
    ...
    tex.set_bbox(dict(facecolor='white',edgecolor='white'))
    fig.savefig(pappath + '/DJF-1.4-MED-cyclone-SLP-evo-PVcross.png',dpi=300,bbox_inches='tight')
    plt.close(fig)
