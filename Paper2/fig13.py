...
labels=['(a) Spring weak','(b) Spring moderate','(c) Spring strong','(d) Autumn weak','(e) Autumn moderate','(f) Autumn strong']
gfig=plt.figure(figsize=(10,5))
ggs = gridspec.GridSpec(nrows=2, ncols=3)

for qq,se in enumerate(seas[1:]):
  for t in ['05_00']:
...
#        print(sim)
        ...
        if sim[-4:]=='clim' or 'check' in sim or 'AT' in sim:
            continue
...
cbax = gfig.add_axes([pos.x0+pos.width, pos.y0, 0.02, pos.height*2])
...
gfig.savefig('/atmosdyn2/ascherrmann/paper/NA-MED-link/composites-normal-runs-med-streamer/MAM-SON-streamer-%s.png'%t,dpi=300,bbox_inches='tight')
plt.close(gfig)
