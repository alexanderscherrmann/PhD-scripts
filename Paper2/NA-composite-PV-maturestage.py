...
for se in seas:
  for amp in amps:
    ...
    for simid2,sim in enumerate(SIMS):
        ...
        slp = slp[loc]
        print(sim,slp,np.min(slp))
        t0 = t[loc[np.where(slp==np.min(slp))[0][0]]]
    ...
    con=ax.contour(LON[0],LAT[:,0],avslp/counter,levels=np.arange(960,1031,5),colors='purple')
#    ax.set_xlim(-20,40)
#    ax.set_ylim(20,65)
    plt.clabel(con, inline=1, fontsize=8, fmt='%.0f')
    cbax = fig.add_axes([0, 0, 0.1, 0.1])
    cbar=plt.colorbar(hc, ticks=pvlevels,cax=cbax)
    func=resize_colorbar_vert(cbax, ax, pad=0.0, size=0.01)
    fig.canvas.mpl_connect('draw_event', func)

    fig.savefig(dwrf + 'image-output/PV-AT-mature-%s-%.1f.png'%(se,amp),dpi=300,bbox_inches='tight')
    plt.close('all')
