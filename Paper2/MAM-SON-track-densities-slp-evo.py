...
SIMS = np.array(SIMS)
Sims2,atids,medids = wrfsims.upper_ano_only()
...
for se in ['MAM','SON']:
    ...
    for amp in ['0.7','1.4','2.1']:
     ...
     for sim in sims2:
        sea = sim[:3]
        if 'not' in sim or 'check' in sim or 'AT' in sim or sim[:-4]=='clim':
           continue
    ...
    f = open(dwrf + 'data/%s-density-data-slp-no-sppt.txt'%se,'wb')
    pickle.dump(save,f)
    f.close()
