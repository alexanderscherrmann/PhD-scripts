...
for simid, sim in enumerate(SIMS):
    ...
    if not sim.startswith('DJF-') or 'no-' in sim or 'AT' in sim or '800' in sim or 'check' in sim:
        continue
...
print(pr(corratslp,corrmedslp))

ax.set_xticks(ticks=np.arange(40,80,10))
ax.set_xticklabels(labels=np.arange(40,80,10),fontsize=14)
#ax.set_yticklabels(labels=np.arange(950,1020,10),fontsize=14)
ax.set_ylabel('Cyclone mininmum SLP [hPa]',fontsize=14)
ax.set_xlabel('Maximal zonal jet velocity [m s$^{-1}$]',fontsize=14)
legend=['Atlantic','Mediterranean']
ax.legend(legend,loc='upper right')
name = pappath + 'jet-intensity-DJF-cyclone-intensity-no-sppt.png'
