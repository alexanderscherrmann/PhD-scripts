...
simsb=['DJF-0.7-no-mp-no-cv','DJF-1.4-no-mp-no-cv']
        ...
        try:
            if t!='01_00':
                ...
                if np.any(tt[loc]==(int(t[1])-1)*24):
                    ...
                    ax.text(lo[loc[loco]]-2,la[loc[loco]]+3,nalab,fontweight='bold',color='white').set_zorder(1000)
                ...
                #    loco = np.where(tt[loc]==(int(t[1])-1)*24)[0][0]
                #    ax.scatter(lo[loc[loco]],la[loc[loco]],marker='*',color='white',zorder=20)
                #    ax.text(lo[loc[loco]]-2,la[loc[loco]]+3,medlab,fontweight='bold',color='white').set_zorder(1000)
        except:
            pass
...
ax = fig.get_axes()[5]
ax.text(0.02,1.02,'no-mp-no-cv perturbation amplitude = 22 m s$^{-1}$', transform=ax.transAxes)

ax = fig.get_axes()[0]
ax.text(0.02,1.02,'no-mp-no-cv perturbation amplitude = 11 m s$^{-1}$',transform=ax.transAxes)

fig.savefig(pappath + 'no-mp-no-cv-weak-mod-evo.png',dpi=300,bbox_inches='tight')
plt.close('all')
        ...
        try:
            if t!='01_00':
                ...
                if np.any(tt[loc]==(int(t[1])-1)*24):
                    ...
                    loco = np.where(tt[loc]==(int(t[1])-1)*24)[0][0]
                    ax.scatter(lo[loc[loco]],la[loc[loco]],marker='*',color='white',zorder=20)
                    ax.text(lo[loc[loco]]-2,la[loc[loco]]+3,'A',fontweight='bold',color='white').set_zorder(1000)
    
                loc=np.where((ID==2) & (tt>=81))[0]
                
                if np.any(tt[loc]==(int(t[1])-1)*24):
                    ax.plot(lo[loc],la[loc],color='white',zorder=2)
                    loco = np.where(tt[loc]==(int(t[1])-1)*24)[0][0]
                    ax.scatter(lo[loc[loco]],la[loc[loco]],marker='*',color='white',zorder=20)
                    ax.text(lo[loc[loco]]-2,la[loc[loco]]+3,'B',fontweight='bold',color='white').set_zorder(1000)
...
cbax = fig.add_axes([x0, pos.y0-0.04, pos.width, 0.01])
cbar=plt.colorbar(hc, ticks=pvlevels,cax=cbax,orientation='horizontal')
pvticklabels[-1]='PVU'
cbar.ax.set_xticklabels(pvticklabels)
plt.subplots_adjust(wspace=0.0,hspace=0)
...
ax = fig.get_axes()[0]
ax.text(0.02,1.02,'no-mp-no-cv perturbation amplitude = 34 m s$^{-1}$',transform=ax.transAxes)

fig.savefig(pappath + 'no-mp-no-cv-strong-evo.png',dpi=300,bbox_inches='tight')
plt.close('all')
