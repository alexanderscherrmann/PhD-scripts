...
for simid, sim in enumerate(SIMS):
    ...
    RH850 = wrf.interplevel(RH,p,850,meta=False)
    ...
    h = ax.contourf(LON,LAT,RH850,cmap=cmap,extend='max',levels=levels)
    ...
    if 'clim-max' in sim:
        ...
        ax.text(35,55,'%d'%slp[loc[np.argmin(slp[loc])]],color='white')
        row +=1
        ax=fi[sea].add_subplot(gs[row,col],projection=ccrs.PlateCarree())
        ax.add_feature(cartopy.feature.GSHHSFeature(scale='low'),zorder=2, edgecolor='black')

        ax.contourf(LON,LAT,RH850,cmap=cmap,extend='max',levels=levels)
        ax.scatter(tlon[loc[np.argmin(slp[loc])]],tlat[loc[np.argmin(slp[loc])]],color='purple',marker='o')
        ax.set_extent([minlon,maxlon,minlat,maxlat])
        ax.text(0,21,'%.1f-center'%amp)
        ax.text(35,55,'%d'%slp[loc[np.argmin(slp[loc])]],color='white')
...
for sea in ['DJF','MAM','SON']:
    ...
    cbax = fi[sea].add_axes([0.9, 0.11, 0.0001, 0.795])
    ...
    fi[sea].savefig(image+ sea + '-low-level-RH-maps.png',dpi=300,bbox_inches='tight')
