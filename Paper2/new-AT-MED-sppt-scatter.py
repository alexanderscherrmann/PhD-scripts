...
colors=['grey','k','k','k','b','r','k']#,'dodgerblue','b','k','k','k','k']
#col200km = ['deepskyblue','limegreen','lightcoral','peru']
ENSW = np.array(['east-','north','south','west-'])
enswmarker = ['o','o','o','+','d','s','x']
markers=['o','o','o','o','1','d','s','x']
ls=' '
for co,ma in zip(colors,enswmarker):
    if co=='grey':
     ax.plot([],[],ls=ls,marker=ma,color=co,alpha=0.4)
     axx.plot([],[],ls=ls,marker=ma,color=co,alpha=0.4)
    else:
     ax.plot([],[],ls=ls,marker=ma,color=co)
     axx.plot([],[],ls=ls,marker=ma,color=co)
...
for simid, sim in enumerate(SIMS):
    ...
    if 'no-' in sim or 'check' in sim or 'AT' in sim or '800' in sim:
        continue
    ...
    if np.any(dist==int(strings[0])):
        dis = int(strings[0])
    if np.any(dist==int(strings[0])) and '0.5' in sim:
        continue
...
#        mark = enswmarker[qq]
    ...
    ax.scatter(maxpv,aminslp,color=acol,marker=mark,alpha=0.4) 
    axx.scatter(deepening,slpmin,color='k',marker=mark)
...
#    MEDSLP = np.append(MEDSLP,slpmin)
    ...
    print(sim,aminslp,slpmin)
    gax.scatter(aminslp,slpmin,color='k')
    if '2.8' in sim:
        gax.scatter(aminslp,slpmin,color='r')
#    ax.scatter(maxpv,slpmin,color=col,marker=mark)
#    if slpmin<995:
#        print(sim,slpmin,aminslp)
...
##        print(sim,aminslp)
#    ax.scatter(maxpv,aminslp,color=acol,marker=mark,alpha=0.4)
#    axx.scatter(deepening,slpmin,color=col,marker=mark)
##    ax.scatter(maxpv,aminslp,color='grey',marker=mark)
##    ATSLP  =np.append(ATSLP,aminslp)
##    MEDSLP = np.append(MEDSLP,slpmin)
#    ax.scatter(maxpv,slpmin,color=col,marker=mark)
...
ax.legend(['NA cyclone','MED cyclone','central','east','north','south','west'],loc='upper right')
name = pappath + 'new-nested-DJF-NA-MED-cyclone-intensity-max-PV-full-onecolor-all-sppt.png'
...
#figg.savefig(pappath + 'new-nested-DJF-NA-deepning-MED-cyclone-intensity-sppt.png',dpi=300,bbox_inches='tight')
#plt.close(figg)


#gax.set_xticklabels(labels=np.arange(950,1010,10),fontsize=14)
#gax.set_yticks(ticks=np.arange(985,1020,5),fontsize=14)
#gax.set_yticklabels(labels=np.arange(985,1020,5),fontsize=14)
#gax.set_ylim(985,1015)
gax.set_xlabel('Atlantic cyclone intensity [hPa]',fontsize=14)
gax.set_ylabel('Med cyclone intensity [hPa]',fontsize=14)
gfig.savefig(pappath + 'new-atlantic-MED-min-slp-no-sppt.png',dpi=300,bbox_inches='tight')
plt.close(gfig)
