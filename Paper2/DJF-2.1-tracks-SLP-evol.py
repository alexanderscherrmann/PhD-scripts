...
for simid,sim in enumerate(SIMS):
    if sim!='DJF-clim-max-U-at-300-hPa-2.1-QGPV':
        continue
    ...
    for col,i in zip(['navy','red'],[1,2]):
        loc = np.where(IDs==i)[0]
        ax.plot(tlon[loc],tlat[loc],color=col)
    ax.set_xlim(-80,60)
    ax.set_ylim(25,80)
#    ax.set_extent([-120,80,20,80])
    figg.savefig(pappath + 'DJF-2.1-AT-MED-cyclone-tracks.png',dpi=300,bbox_inches='tight')
    plt.close(figg)
    ...
    ax.plot(t[loc],slp[loc],color='k')

    ax.set_xlabel('simulation time [h]')
    ax.set_xticks(ticks=np.arange(24,193,24))
    ax.set_xlim(t[loc[0]],t[loc[-1]])
    ...
    fig.savefig(pappath + '/DJF-2.1-MED-cyclone-SLP-evo-PVcross.png',dpi=300,bbox_inches='tight')
    plt.close(fig)
