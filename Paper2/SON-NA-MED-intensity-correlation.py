...
for simid, sim in enumerate(SIMS):
    ...
    ax.scatter(aminslp,slpmin,color=col,marker=mark)
    ATSLP  =np.append(ATSLP,aminslp)
    MEDSLP = np.append(MEDSLP,slpmin)
#    ax.scatter(maxpv,slpmin,color='blue',marker=mark)


ax.set_ylabel('Med. cyclone mininmal SLP [hPa]')
ax.set_xlabel('NA cyclone minimal SLP [hPa]')

print(pr(ATSLP,MEDSLP))
ax.legend(legend,loc='upper left')

#ax.legend(['NA cyclone','MED cyclone'],loc='upper right')
name = pappath + 'SON-NA-MED-cyclone-intensity-correlation.png'
