...
for k,sim in enumerate(sims):
    ...
    for l,t in enumerate(times):
        ...
        if t=='03_00' or t=='05_00' or t=='07_00' or t=='09_00':
            loc=np.where(ID==1)[0]
...
ax = fig.get_axes()[5]
ax.text(0.01,1.02,'Spring perturbation amplitude = 22 m s$^{-1}$', transform=ax.transAxes)

ax = fig.get_axes()[0]
ax.text(0.01,1.02,'Autumn perturbation amplitude = 22 m s$^{-1}$',transform=ax.transAxes)

fig.savefig(pappath + 'MAM-SON-1.4-dynamics.png',dpi=300,bbox_inches='tight')
plt.close('all')
