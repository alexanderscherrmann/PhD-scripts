...
for simid, sim in enumerate(SIMS):
    ...
    QV = wrf.getvar(ic,'QVAPOR')*1000
    p = wrf.getvar(ic,'pressure')

    QV850 = wrf.interplevel(QV,p,850,meta=False)
    ...
    h = ax.contourf(LON,LAT,QV850,cmap=cmap,extend='max',levels=levels)
    ...
    if 'clim-max' in sim:
        ...
        ax.contourf(LON,LAT,QV850,cmap=cmap,extend='max',levels=levels)
        ax.scatter(tlon[loc[np.argmin(slp[loc])]],tlat[loc[np.argmin(slp[loc])]],color='purple',marker='o')
        ax.set_extent([minlon,maxlon,minlat,maxlat])
        ax.text(0,21,'%.1f-center'%amp)
        ax.text(35,55,'%d'%slp[loc[np.argmin(slp[loc])]])
...
for sea in ['DJF','MAM','SON']:
    ...
    fi[sea].savefig(image+ sea + '-low-level-QV-maps.png',dpi=300,bbox_inches='tight')
