...
pvpos = [[93,55],[95,56],[140,73],[93,55]]
...
pappath = '/home/ascherrmann/publications/upstream-downstream-connection/'
Fig = plt.figure(figsize=(12,20))
Gs = gridspec.GridSpec(nrows=4, ncols=5)
#Labels=[['(a)','(b)','(c)'],['(d)','(e)','(f)'],['(g)','(h)','(i)'],['(j)','(k)','(l)']]
Labels=[['(a) DJF','(b) DJF','(c) DJF'],['(d) MAM','(e) MAM','(f) MAM'],['(g) JJA','(h) JJA','(i) JJA'],['(a)','(b)','(c)']]

storestuff=dict()
xtext,ytext=0.03,1.03
#xtext,ytext=0.03,0.9
for pvpo,sea in zip(pvpos,seasons):
    q = np.where(np.array(seasons)==sea)[0][0]
    ### figure
    per = ds(wrfd + sea + '-clim/wrfout_d01_2000-12-01_00:00:00','r')
    if sea=='SON':
        per = ds(wrfd + 'DJF-clim-max-U-at-300-hPa-1.4-QGPV/wrfout_d01_2000-12-01_00:00:00','r')
    storestuff[sea] = dict()
...
#    text.set_bbox(dict(facecolor='white',edgecolor='white'))
    ...
    if upercross.shape==storestuff['DJF']['U'].shape:
        h1=nax.contour(xcoord,vcross_pper,upercross-storestuff['DJF']['U'],levels=np.array([2,5,10,15,20]),colors='black',linewidths=0.5,zorder=1000.)
        plt.clabel(h1,inline=True,fmt='%d',fontsize=6)
        h2=nax.contour(xcoord,vcross_pper,upercross-storestuff['DJF']['U'],levels=np.array([-20,-15,-10,-5,-2]),colors='black',linestyles='--',linewidths=0.5,zorder=1000)
        plt.clabel(h2,inline=True,fmt='%d',fontsize=6)
        print(np.max(upercross-storestuff['DJF']['U']),np.min(upercross-storestuff['DJF']['U']))
    else:
        h1=nax.contour(xcoord[:,:-1],vcross_pper[:,:-1],upercross[:,:-1]-storestuff['DJF']['U'],levels=np.array([2,5,10,15,20]),colors='black',linewidths=0.5)
        plt.clabel(h1,inline=True,fmt='%d',fontsize=6)
        h2=nax.contour(xcoord[:,:-1],vcross_pper[:,:-1],upercross[:,:-1]-storestuff['DJF']['U'],levels=np.array([-20,-15,-10,-5,-2]),colors='black',linestyles='--',linewidths=0.5)
        plt.clabel(h2,inline=True,fmt='%d',fontsize=6)
...
Fig.savefig(pappath + 'fig02-to-be-cut.png',dpi=300,bbox_inches='tight')
plt.close('all')
