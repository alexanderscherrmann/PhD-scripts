...
sims=['DJF-clim','DJF-clim-max-U-at-300-hPa-1.4-QGPV']

cmap=matplotlib.cm.coolwarm
levels=np.arange(310,361,5)
...
#        PV300ref = wrf.interplevel(PVref,Pref,300,meta=False)
        TH2PVU = wrf.interplevel(THref[10:],PVref[10:],2,meta=False)
        ...
        hc=ax.contourf(lon,lat,TH2PVU,cmap=cmap,levels=levels,extend='both')
...
cbar.ax.set_xticklabels(np.append('TH',levels[1:]))
#func=resize_colorbar_hor(cbax, ax, pad=0.0, size=0.005)
#fig.canvas.mpl_connect('draw_event', func)
plt.subplots_adjust(wspace=0.0,hspace=0)

fig.savefig(pappath + 'DJF-TH-ref-1.4-dynamics.png',dpi=300,bbox_inches='tight')
plt.close('all')
...
#        hc=ax.contourf(lon,lat,PV300ref,cmap=pvcmap,norm=pvnorm,extend='both',levels=pvlevels)
        hc=ax.contourf(lon,lat,TH2PVU,cmap=cmap,extend='both',levels=levels)
        plt.clabel(h2,inline=True,fmt='%d',fontsize=6)
...
fig.savefig(pappath + 'DJF-TH-0.7-2.1-dynamics.png',dpi=300,bbox_inches='tight')
plt.close('all')
