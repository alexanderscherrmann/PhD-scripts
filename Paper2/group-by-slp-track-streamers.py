...
for simid,sim in enumerate(SIMS):
   ...
   for i in [1,2]:
       ...
       if i==1:
        ...
        ax.scatter(tlon[loc[loco]],tlat[loc[loco]],color=col,marker='*',s=50,zorder=10)
           ...
           if np.min(slp[loc])<996:
               ax2.plot(tlon[loc],tlat[loc],color=col,linestyle=ls)
               ax2.scatter(tlon[loc[loco]],tlat[loc[loco]],color=col,marker='*',s=50,zorder=10)
           else:
   ...
   data = ds(dwrf + sim + '/wrfout_d01_2000-12-05_00:00:00')
   pv = wrf.getvar(data,'pvo')
   p = wrf.getvar(data,'pressure')
   pv300 = wrf.interplevel(pv,p,300,meta=False)

   if counter1==0 and counter2==0:
       density1=np.zeros_like(pv300)
       density2=np.zeros_like(pv300)

   if np.min(slp[locc])<996:

       ax2.contour(LON[0],LAT[:,0],pv300,levels=[2],colors=col,linewidths=0.5)
       density1[pv300>=2]+=1
       counter1 +=1
   else:
...
hb = ax3.contourf(LON[0],LAT[:,0],density2*100/counter2,levels=clevels,cmap=cmap)
hb = ax2.contourf(LON[0],LAT[:,0],density1*100/counter1,levels=clevels,cmap=cmap)
...
ax2.set_xticklabels(['40$^{\circ}$W','20$^{\circ}$W','0$^{\circ}$E','20$^{\circ}$E','40$^{\circ}$E'])
ax2.set_yticks([20,35,50,65])
ax2.set_yticklabels(['20$^{\circ}$N','35$^{\circ}$N','50$^{\circ}$N','65$^{\circ}$N'])


ax3.set_xlim(-45,45)
ax3.set_ylim(20,65)
ax3.set_xticks([-40,-20,0,20,40])
ax3.set_xticklabels(['40$^{\circ}$W','20$^{\circ}$W','0$^{\circ}$E','20$^{\circ}$E','40$^{\circ}$E'])
ax3.set_yticks([20,35,50,65])
ax3.set_yticklabels(['20$^{\circ}$N','35$^{\circ}$N','50$^{\circ}$N','65$^{\circ}$N'])
...
fig.savefig(pappath + 'spread-at-996-hPa-med-cyclones.png',dpi=300,bbox_inches='tight')
plt.close('all')
