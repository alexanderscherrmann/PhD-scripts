...
for simid, sim in enumerate(SIMS):
    ...
    h = ax.contourf(LON,LAT,PV850,cmap=cmap,norm=norm,extend='both',levels=levels)
    ...
    if 'clim-max' in sim:
        ...
        ax.contourf(LON,LAT,PV850,cmap=cmap,norm=norm,extend='both',levels=levels)
        ax.scatter(tlon[loc[np.argmin(slp[loc])]],tlat[loc[np.argmin(slp[loc])]],color='purple',marker='o')
        ax.set_extent([minlon,maxlon,minlat,maxlat])
        ax.text(0,21,'%.1f-center'%amp)
        ax.text(35,55,'%d'%slp[loc[np.argmin(slp[loc])]])
...
for sea in ['DJF','MAM','SON']:
    ...
    fi[sea].savefig(image+ sea + '-low-level-PV-maps.png',dpi=300,bbox_inches='tight')
