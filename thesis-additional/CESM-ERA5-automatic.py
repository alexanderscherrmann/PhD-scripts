...
for sea in seasons[1:2]:
    ...
    d = ds(ics + 'CESM-ERA5-%s-clim/met_em.d01.2000-12-01_00:00:00.nc'%sea,'r')
    ...
    os.system('module load dyn_tools;module load conda/2022;source activate iacpy3_2022; bash /atmosdyn2/ascherrmann/scripts/WRF/pvinv-cart/CESM-ERA5-automatic.sh %s %d %d %d %s %s '%(sea,int(xmax+1),int(ymax+1),int(ght300[ymax,xmax]),'max',0))
...
#            os.system('module load dyn_tools;module load conda/2022;source activate iacpy3_2022; bash /atmosdyn2/ascherrmann/scripts/WRF/pvinv-cart/CESM-ERA5-automatic.sh %s %d %d %d %s %s '%(sea,int(xtmp),int(ytmp),int(ztmp),n,k))
