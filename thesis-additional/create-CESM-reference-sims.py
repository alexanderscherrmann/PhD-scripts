...
for sea in seasons[1:]:
  if sea=='JJA':
      continue
  for period in ['2010','2040','2070','2100'][::3]:#,'2070']:#,'2100']:

    if not os.path.isdir(pi + 'CESM-%s-'%period + sea + '-clim'):
        os.mkdir(pi + 'CESM-%s-'%period + sea + '-clim')

    pa = 'CESM-%s-'%period + sea + '-clim/'
    ...
    p3dCESM = '/atmosdyn2/ascherrmann/015-CESM-WRF/%s/member-mean/'%sea

    for paraP, va in zip(paramP,varp):
        Vars[paraP] = readcdf(p3dCESM + 'all-member-mean-0.5-%s-%s.nc'%(period,sea),va)[0]
    
    lonP = readcdf(p3dCESM + 'all-member-mean-0.5-%s-%s.nc'%(period,sea),'lon')
    latP = readcdf(p3dCESM + 'all-member-mean-0.5-%s-%s.nc'%(period,sea),'lat')
    ...
    p2dCESM = '/atmosdyn2/ascherrmann/015-CESM-WRF/%s/member-mean/'%sea

    for para2D,va in zip(param2c,var2c):
        Vars[para2D] = readcdf(p2dCESM + 'all-member-surface-mean-0.5-%s-%s.nc'%(period,sea),va)[0]

    lon2D = readcdf(p2dCESM + 'all-member-surface-mean-0.5-%s-%s.nc'%(period,sea),'lon')
    lat2D = readcdf(p2dCESM + 'all-member-surface-mean-0.5-%s-%s.nc'%(period,sea),'lat')
