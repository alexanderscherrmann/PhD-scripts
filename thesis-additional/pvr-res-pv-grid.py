...
prodi = dict()
if not os.path.isfile('/home/ascherrmann/thesis-images/gridmap-res.txt'):
 for q, date in enumerate(datadi.keys()):
    ...
    for k in range(len(idp)):
        for l in range(len(tralon[0])):
            ...
            for pv in np.append(pvrs[:-1],'locres2')[-1:]:
 ...
 save['res'] = gridmap['locres2']
 f = open('/home/ascherrmann/thesis-images/gridmap-res.txt','wb')
 pickle.dump(save,f)
 f.close()
else:
    f = open('/home/ascherrmann/thesis-images/gridmap-res.txt','rb')
    save = pickle.load(f)
    f.close()
...
gridmap = dict()
gridmap['locres2'] = save['res']
gridmap['counter']=counter
loc = np.where(counter!=0)

for pv in np.append(pvrs,'locres2')[-1:]:
...
psave = '/home/ascherrmann/thesis-images/'
maxv = 3000
minv = 800
...
for pv in ['locres2']:#np.append(np.append(np.append('PVRT','counter'),pvrs),'locres2'):
    minv=-0.35
    maxv=0.35
    steps=0.05
    ...
    if pv=='locres2':
        ap = plt.cm.terrain #PuBuGn
        minv = 0
        maxv = 0.5
        steps= 0.05
        pvr_levels = np.round(np.arange(minv,maxv+0.0001,steps),2)
...
#    ax.add_feature(cartopy.feature.GSHHSFeature(scale='low'),zorder=10, edgecolor='black')
    ...
    lc=ax.contourf(LON,LAT,gridmap[pv],levels=pvr_levels,cmap=cmap,norm=norm,extend='max')
...
#        lc.cmap.set_over('red')
    ...
    cbar.ax.set_yticklabels(labels=np.append('PVU/h',pvr_levels[1:]))#,fontsize=8)
...
nbin = 64
ax.set_xlabel(xlab)
ax.set_ylabel('residual [PVU]')

pv='locres2'
h = ax.hist2d(xal,abs(yal),range=[xlim,ylim],cmin=10,norm=matplotlib.colors.LogNorm(),bins=nbin,cmap='nipy_spectral')
ticklabels = np.array([10,100,1000,1e4])
labels = np.array(['10','100',r'10$^3$',r'10$^4$'])
cbax = fig.add_axes([0, 0, 0.1, 0.1])
cbar=plt.colorbar(h[3], ticks=ticklabels,cax=cbax)
func=resize_colorbar_vert(cbax, ax, pad=0.00, size=0.01)
fig.canvas.mpl_connect('draw_event', func)
ax.set_aspect('auto')
