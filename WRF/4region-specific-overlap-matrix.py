...
#lonsr = np.where((LON>=minrrlon) & (LON<=maxrrlon))[0]
#latsr = np.where((LAT<=maxrrlat) & (LAT>=minrrlat))[0]

#lorr0,lorr1,larr0,larr1 = lonsrr[0],lonsrr[-1]+1,latsrr[0],latsrr[-1]+1
...
pool = dict()
seasons = list(d.keys())
regions = list(d['DJF'].keys())

for sea in d.keys():
    pool[sea] = dict()
    for re in d[sea].keys():
        pool[sea][re] = np.array([])
        for ID in d[sea][re]:
            dirs = '%06d'%ID

            for kk in range(65):
                pool[sea][re] = np.append(pool[sea][re],'%06d,%02d'%(int(dirs),kk))
...
#    refrr = refr.variables['mask'][kk,larr0:larr1,lorr0:lorr1]
...
#    rrlabels = np.array([])
#    rrlens = np.array([])
#    for v in np.unique(refrr)[1:]:
#        rrlabels = np.append(rrlabels,v)
#        rrlens = np.append(rrlens,len(np.where(refrr=v)[0]))
...
#    overlap[currenttype]['ridge2'] = []
    ...
    rov[refr==rlabel] +=1 
#    rrov = np.zeros_like(refrr)
#    rrov[refrr==rrlabel]+=1
...
#        rrmask=r.variables['mask'][k,larr0:larr1,lorr0:lorr1]
...
f = open(ps + '%s-%s-general-types-%.1f-ridge-streamer-types-%.1f.txt'%(sea,re,sizecheck,checkval),'wb')
pickle.dump(overlap,f)
f.close()


f = open(ps + '%s-%s-general-types-size-%.1f-streamer-types-%.1f.txt'%(sea,re,sizecheck,checkval),'wb')
