...
#        continue
    if d=='DJF-clim-max-U-at-300hPa-hourly-2.1QG' or d=='sat-DJF-clim-max-U-at-300hPa-hourly-2.1QG':
        continue
    if not d.startswith('DJF-clim-max') and not d.startswith('MAM-clim-max') and not d.startswith('JJA-clim-max') and not d.startswith('SON-clim-max'):# and not d.startswith('DJF-clim-double') and not d.startswith('DJF-L300') and not d.startswith('DJF-L500'):
        continue

    slptxt = np.loadtxt(tracks + d + '-minMedSLP.txt')[20:,1]
    tra = np.loadtxt(tracks + d + '-filter.txt',skiprows=4)
    t = tra[:,0]
    tlon = tra[:,1]
    tlat = tra[:,2]
    slp = tra[:,3]
...
#        aloc.append(loc)
    ...
    ax2.text(aminslp,slpmin,'%d'%(counter))
