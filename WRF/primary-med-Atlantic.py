...
#markers=['o','o','o','o','x','+']
legend=['DJF','MAM','JJA','SON','DJF-200km','DJF-300km','DJF-500km']
...
#    if d=='DJF-clim-max-U-at-300-hPa-hourly-2.1-QGPV' or d=='sat-DJF-clim-max-U-at-300hPa-hourly-2.1QG':
#        continue
#    if not d.startswith('DJF-clim-max') and not d.startswith('MAM-clim-max') and not d.startswith('JJA-clim-max') and not d.startswith('SON-clim-max') and not d.startswith('DJF-L-300') and not d.startswith('DJF-L-500') and not d.startswith('DJF-clim-double') and not d.startswith('DJF-L-800'):
#        continue
#
#    try:
#        simid = np.where(SIMS==d)[0][0]
#    except:
#        continue
...
name = dwrf + 'image-output/MED-cyclone-PV-anomaly-SLP-scatter-min-of-all-tracks-in-MED.png'
...
name = dwrf + 'image-output/Atlantic-MED-cyclone-SLP-scatter-min-of-all-tracks-in-MED.png'
...
name = dwrf + 'image-output/MED-cyclone-genesis-time-scatter-min-of-all-tracks-in-MED.png'
...
name = dwrf + 'image-output/MED-cyclone-Atlantic-deepening-6h-scatter-min-of-all-tracks-in-MED.png'
