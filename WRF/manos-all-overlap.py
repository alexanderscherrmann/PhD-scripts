...
if True:
    ...
    overlap[currenttype]['ids'] = np.array([dirs])
    ...
    for deli, p in enumerate(pool):
        dirs=p[:6]
        if np.any(overlap[currenttype]['ids']==dirs):
            continue
        k = int(p[-2:])
        ...
        overlap[currenttype]['ids'] = np.append(overlap[currenttype]['ids'],dirs)
...
f = open(ps + 'test1.txt','wb')
pickle.dump(overlap,f)
f.close()


f = open(ps + 'test2.txt','wb')
