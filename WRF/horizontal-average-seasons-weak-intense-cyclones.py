    ...
    for wi in which[:]:
      ...
      for ll in [50]:#, 100, 150, 200]:
        ...
        for wq,we in zip(wl,when):
            plots['TH850'] = data[sea][wi][ll][we]['TH850']
            plots['U300hPa'] = data[sea][wi][ll][we]['U300hPa']
            plots['PV300hPa'] = data[sea][wi][ll][we]['PV300hPa']
            LSP24 = data[sea][wi][ll][we]['lprecip-24h']
            CP24 = data[sea][wi][ll][we]['cprecip-24h']
            plots['omega500'] = data[sea][wi][ll][we]['omega500']
            cycfreq = data[sea][wi][ll][we]['cycfreq']
            SLPcycfreq = data[sea][wi][ll][we]['SLPcycfreq']
            wcba = data[sea][wi][ll][we]['wcbascfreq']
            wcbo = data[sea][wi][ll][we]['wcbout400freq']
            plots['MSL'] = data[sea][wi][ll][we]['MSL']/100
...
#              if var=='MSL':
             if var=='kPV300hPa':
                ...
                if var=='TH850':
                    ax.contour(LON[lons],LAT[lats],wcbo*100,linewidths=1,alpha=1,colors='cyan',levels=np.arange(20,80,10))
                if var=='MSL':
                ...
                name = nam + '-for-' + wi[:-4] + '-' + wq + '-' + sea + '-' + '%d'%ll +'.png'
    ...
    ll=50
    for wq,we in zip(wl,when):
        plots['wcboutdiff'] = data[sea]['intense-cyclones.csv'][50][we]['wcbout400freq'] - data[sea]['weak-cyclones.csv'][50][we]['wcbout400freq']
        plots['cycfreqdiff'] = data[sea]['intense-cyclones.csv'][50][we]['cycfreq'] - data[sea]['weak-cyclones.csv'][50][we]['cycfreq']
        ...
        for var, nam in zip(['wcboutdiff','cycfreqdiff'],['wcbout-diff','cycfreq-diff']):
            if not os.path.isdir(pi + var + '/'):
                os.mkdir(pi + var)
            if not os.path.isdir(pi + var + '/' + '%d'%ll):
                os.mkdir(pi + var + '/' + '%d'%ll)
