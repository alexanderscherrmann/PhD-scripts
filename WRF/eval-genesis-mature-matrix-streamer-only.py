...
pi = '/atmosdyn2/ascherrmann/013-WRF-sim/image-output/' + setto + '-matrix-%.1f-%s-streamer/'%(ovval,frac)
...
a = 0
f = open(ps + setto + '-full-overlap-matrix-%.1f-%s-streamer-only.txt'%(ovval,frac),'rb')
d = pickle.load(f)
f.close()
...
#            print(date,dirs,kk)
        ...
        fig.savefig(pi + 'PV-streamer-only' + fi + '.png',dpi=300,bbox_inches='tight')
        plt.close('all')
