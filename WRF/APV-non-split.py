...
pload = '/atmosdyn2/ascherrmann/012-WRF-cyclones/' 
...
pllegend = ['TOT','|CONVT|>|TURBT|','|TURBT|>|CONVT|', 'CONVM', 'TURBM','RAD','LS']
...
for uyt, txt in enumerate(traj):
    ...
    tt = np.loadtxt(pload + txt,skiprows=5)
                ...
                for wt, ru in enumerate(plotvars):
                    ...
                    for xx in range(len(sq)):
                        if sq[xx]>0:
                            ...
                            if ru=='PVR-T':
                                if (abs(np.sum(tmpdata['PVRCONVT'][idp,xx]))>=(abs(np.sum(tmpdata['PVRTURBT'][idp,xx])))):
...
#                ax.axvline(htzeta[0],color='grey',ls='-')
                ax.set_xlim(xlim)
                ax.set_ylim(ylim)
                ax.set_xticks(ticks=np.arange(-1 * H,1,6))
                ax.set_yticks(ticks=np.arange(-0.25,2.51,0.25))
...
#        if(date=='20171214_02-073'):
        ...
        fig.savefig(pload + name,dpi=300,bbox_inches="tight")
        plt.close()
