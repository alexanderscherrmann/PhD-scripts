...
pi = pl + 'ics/'
pd = '/home/ascherrmann/scripts/WRF/PV-overlap-data/'
...
for day,strength in zip(['4'],['italy']):
    if not os.path.isdir(pi + day + strength + '-clim'):
        os.mkdir(pi + day + strength + '-clim')

    pa = day + strength + '-clim/'
    ...
    for va in varname:
        Vars[va] = readcdf(pd + day + sf,va)[0]	
    for va,par in zip(varsfnn,varssfn):
        Vars[par] = readcdf(pd + day+ sf,va)[0]
    
    lonsoil = readcdf(pd + day + sf,'lon')
    latsoil = readcdf(pd + day + sf,'lat')
    ###
...
#    latN = readcdf(pd + 'Bmean-' + strength,'lat')
    ...
    for paraP, va in zip(paramP,varp):
        Vars[paraP] = readcdf(pd+ day + pl,va)[0]
    
    lonP = readcdf(pd+day+pl,'lon')
    latP = readcdf(pd+day +pl,'lat')
