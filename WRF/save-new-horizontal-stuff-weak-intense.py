...
VAR = ['PV300hPa','U300hPa','cycfreq','TH850','omega950','omega900','omega850','omega800','lprecip-06h','lprecip-12h','lprecip-18h','lprecip-24h','cprecip-06h','cprecip-12h','cprecip-18h','cprecip-24h','SLPcycfreq','omega500','MSL','wcbascfreq','wcbout500freq','wcbout400freq']
    ...
    for wi in which[::2]:
      ...
      for ll in [50, 100, 150, 200]:
        ...
        for we in when:
            ...
            save[sea][wi][ll][we]['ymature'] = np.array([])
            save[sea][wi][ll][we]['xmature'] = np.array([])
            save[sea][wi][ll][we]['wcbcounter'] = 0
...
#                    save[sea][wi][ll][we]['c'+pco]+=PREDATA.variables['CP'][0,la0:la1,lo0:lo1]
#                    save[sea][wi][ll][we]['l'+pco]+=PREDATA.variables['LSP'][0,la0:la1,lo0:lo1]
#
...
#                save[sea][wi][ll][we]['xmature'] = np.append(save[sea][wi][ll][we]['xmature'],np.where(matloc!=0)[1])
#                save[sea][wi][ll][we]['ymature'] = np.append(save[sea][wi][ll][we]['ymature'],np.where(matloc!=0)[0])
...
#                    save[sea][wi][ll][we]['wcbascfreq'][wcbasc!=0] += 1
#                    save[sea][wi][ll][we]['wcbout400freq'][wcbout4!=0] += 1
#                    save[sea][wi][ll][we]['wcbout500freq'][wcbout5!=0] += 1
#                    save[sea][wi][ll][we]['wcbcounter']+=1

                #Pminposition = CM.variables['PMIN'][0,0,la0:la1,lo0:lo1]

                save[sea][wi][ll][we]['cycfreq'][mask!=0]+=1
                save[sea][wi][ll][we]['SLPcycfreq'][mask!=0]+=MSL[mask!=0]
                save[sea][wi][ll][we]['MSL']+=MSL
                ...
                save[sea][wi][ll][we]['omega950'] += omega950
                save[sea][wi][ll][we]['omega900'] += omega900
                save[sea][wi][ll][we]['omega850'] += omega850
                save[sea][wi][ll][we]['omega800'] += omega800
                save[sea][wi][ll][we]['omega500'] += omega500

                save[sea][wi][ll][we]['TH850'] += th850hpa
                save[sea][wi][ll][we]['PV300hPa'] += pv300hpa
                save[sea][wi][ll][we]['U300hPa'] += u300hpa

            for var in VAR[:-3]:
                save[sea][wi][ll][we][var]/=ll


            save[sea][wi][ll][we]['wcbascfreq']/=save[sea][wi][ll][we]['wcbcounter']
            save[sea][wi][ll][we]['wcbout400freq']/=save[sea][wi][ll][we]['wcbcounter']
            save[sea][wi][ll][we]['wcbout500freq']/=save[sea][wi][ll][we]['wcbcounter']

            save[sea][wi][ll][we]['xmature']=save[sea][wi][ll][we]['xmature'].astype(int)
            save[sea][wi][ll][we]['ymature']=save[sea][wi][ll][we]['ymature'].astype(int)
...
#                precid24 = nd*24-23
#
##                precid = [precid06,precid12,precid18,precid24]
#                precid = [precid24]
#                for pco, prec in zip(precidorder,precid):
#                    dhours = np.arange(prec,precid00+1)
#                    dhours /= 24
...
#f = open(ps + 'most-intense-moderate-weak-average-fields.txt','wb')
f = open(ps + 'new-weak-intense-fields.txt','wb')
pickle.dump(save,f)
f.close()
