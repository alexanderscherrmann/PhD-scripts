...
f = open(ps + 'Atlantic-cyclones-cluster.txt','rb')
data = pickle.load(f)
f.close()
...
if not os.path.isdir(pi + 'pre-atlantic-cyclones/'):
    os.mkdir(pi + 'pre-atlantic-cyclones/')

sea = 'DJF'
wi = 'intense-cyclones.csv'
for xla,his,r,b,xtick in zip(xlab,histvar,ranges,bins,xticks):
 for we in data['DJF']['intense-cyclones.csv'][200].keys():
    if np.any(np.array(whenplot)==we):
        ...
        for ll in [200]:            
            ...
            for q,ax,clus in zip(range(len(axes)),axes,list(data[sea][wi][ll][we].keys())):
                ax.text(0.05,0.9,clus,transform=ax.transAxes,fontsize=6,fontweight='bold')

                ax.hist(data[sea][wi][ll][we][clus][his],bins=b,range=r,facecolor='grey',edgecolor='k',alpha=1)
                
                ax.set_xlim(r)
                ax.set_ylim(0,0.1*ll)
