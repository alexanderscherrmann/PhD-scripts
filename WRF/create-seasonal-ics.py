...
#paths = ['ref-mean-large/','ref-overlap-large/']
#paths=['ref-mean-medium/','ref-overlap-medium/']
...
for sea in seasons:
    if not os.path.isdir(pi + sea + '-clim'):
        os.mkdir(pi + sea + '-clim')
    ...
    for va in varname:
        Vars[va] = readcdf(pd + 'sfmean-' + sea + '.nc',va)[0]	
    
    lonsoil = readcdf(pd + 'sfmean-' + sea + '.nc','lon')
    latsoil = readcdf(pd + 'sfmean-' + sea + '.nc','lat')
    ###
    ### 2D fields from thermo era5 data
    ###
    
    for va in varname2:
        Vars[va] = readcdf(pd + 'Bmean-' + sea,va)[0]
    
    lonN = readcdf(pd + 'Bmean-' + sea,'lon')
    latN = readcdf(pd + 'Bmean-' + sea,'lat')
    ...
    for paraP, va in zip(paramP,varp):
        Vars[paraP] = readcdf(pd+'plmean-' + sea + '.nc',va)[0]
    
    lonP = readcdf(pd+'plmean-' + sea + '.nc','lon')
    latP = readcdf(pd+'plmean-' + sea + '.nc','lat')
