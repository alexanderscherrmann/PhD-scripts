...
cmap = ListedColormap(np.array(['navy','dodgerblue','lightskyblue','salmon','lightcoral','indianred','crimson','red','darkorange','orange','gold','yellow','yellowgreen','limegreen','olivedrab','forestgreen']))
        #,ListedColormap(np.array([[250, 120, 0],[250, 180, 0],[250, 210, 5],[255, 245, 5],[190, 220, 15],[120, 200, 15],[100, 150, 20],[20, 130, 20]])/255.))
#cmap.set_under(np.array([0,   100, 255])/255.)
#cmap.set_upper(np.array([208, 232, 178])/255.)
...
for wi in which:
    ...
    for d,lonc,latc in zip(sel[we].values,sel['lon'].values,sel['lat'].values):
    ...
    name = 'composite-PV-crosssection-' + wi[:-4] + '.png'
