...
f = open(dwrf + 'data/CESM-jet-check-density-data-slp-%s.txt'%sea,'rb')
d = pickle.load(f)
f.close()
trackdens = dict()
for row,perio in enumerate(['ERA5','2010','2040','2070','2100'][-1:]):
...
#                    medtrd[perio][am][k][35:64,230:261]=0
        ...
        if perio=='ERA5':
            tex = ax.text(0.03,0.05,'%s %d %s %s'%(labels[3*row+col],len(list(attrd[perio][am].keys())),perio,strength[col]),transform=ax.transAxes,zorder=1000)
        else:
            tex = ax.text(0.03,0.05,'%s %d CESM-%s %s'%(labels[3*row+col],len(list(attrd[perio][am].keys())),perio,strength[col]),transform=ax.transAxes,zorder=1000)
...
#                axx.set_yticklabels(np.append('',np.arange(970,1016,10)))
        ...
        if perio=='ERA5':
            tex = axx.text(0.4,0.1,'%s %s %s'%(labels[3*row+col],perio,strength[col]),transform=axx.transAxes,zorder=1000)
        else:
            tex = axx.text(0.4,0.1,'%s CESM-%s %s'%(labels[3*row+col],perio,strength[col]),transform=axx.transAxes,zorder=1000)
        ...
        if perio=='ERA5':
            tex = axxx.text(0.03,0.1,'%s %s %s'%(labels[3*row+col],perio,strength[col]),transform=axxx.transAxes,zorder=1000)
        else:
            tex = axxx.text(0.03,0.1,'%s CESM-%s %s'%(labels[3*row+col],perio,strength[col]),transform=axxx.transAxes,zorder=1000)
        tex.set_bbox(dict(edgecolor='white',facecolor='white'))
...
figg.savefig('/atmosdyn2/ascherrmann/015-CESM-WRF/images/alltrackdens-jet-check-%s.png'%sea,dpi=300,bbox_inches="tight")
