...
labels=['(a) E2010 weak','(b) E2010 moderate','(c) E2010 strong','(d) C2010 weak','(e) C2010 moderate','(f) C2010 strong']
gfig=plt.figure(figsize=(10,5))
ggs = gridspec.GridSpec(nrows=2, ncols=3)

for qq,perio in enumerate(['ERA5','2010']):
  for t in ['05_00']:
   for qw, amp in enumerate(amps):
    ...
    for simid,sim in enumerate(SIMS):
        if '800' in sim or not perio in sim or not str(amp) in sim: 
            continue
        print(sim)
...
gfig.savefig('/atmosdyn2/ascherrmann/015-CESM-WRF/streamers/ERA5-PD-CESM-PD-streamer-400km.png',dpi=300,bbox_inches='tight')
plt.close(gfig)
