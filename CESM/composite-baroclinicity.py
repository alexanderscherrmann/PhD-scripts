...
thl = np.arange(270,301,3)
pvcmap,pvlevels,pvnorm,ticklabels=matplotlib.cm.coolwarm,thl,BoundaryNorm(thl,256),thl#np.arange(300,346,3)
...
for wq,perio in enumerate(period):
    for t in ['05_00']:#['03_00','03_12','04_00','04_12','05_00','05_12','06_00','06_12','07_00','07_12','08_00','08_12','09_00']:
        ...
        for wl,ampl,amp in zip(range(3),['weak','moderate','strong'],amps):
            ...
            avth = np.zeros_like(ref.variables['T'][0,0])
...
#                    continue
                ...
                th850 = wrf.interplevel(th,p,900,meta=False)
          ...
          #  ax.set_ylim(20,65)
            ...
            hb = axx.contourf(LON[0],LAT[:,0],avth/counter,levels=pvlevels,cmap=pvcmap,norm=pvnorm)
            fax.contourf(LON[0],LAT[:,0],avth/counter,levels=pvlevels,cmap=pvcmap,norm=pvnorm)
          #  kk=4
    ...
    #    qk.set_bbox(dict(facecolor='white',edgecolor='white'))
        ...
        cbar.ax.set_yticklabels(labels=np.append(ticklabels[:5],np.append(np.array(ticklabels[5:-1]).astype(int),'K')))
...
#        gfig.savefig('/atmosdyn2/ascherrmann/015-CESM-WRF/%s-med-streamer-composites-%s.png'%(perio,t),dpi=300,bbox_inches='tight')
#        plt.close(gfig)
...
fullfig.savefig('/atmosdyn2/ascherrmann/015-CESM-WRF/DJF-baroclinicity-composites-%s.png'%t,dpi=300,bbox_inches='tight')
plt.close('all')
