...
clim = dict()
for perio in period:
    clim[perio]=ds('/atmosdyn2/ascherrmann/013-WRF-sim/CESM-%s-DJF-clim/wrfout_d01_2000-12-01_00:00:00'%perio)
...
#        if perio in si:
#            slpclim = clim[perio].variables['MSLP'][0,np.where(abs(LAT-mlat)==np.min(abs(LAT-mlat)))[0][0],np.where(abs(LON-mlon)==np.min(abs(LON-mlon)))[0][0]]
#            medslpdi[perio+'-jet'].append(slpmin-slpclim)
#            print(si,perio,slpclim,slpmin)
#             medslpdi[perio].append(slpmin-slpclim)
...
f=open('/atmosdyn2/ascherrmann/015-CESM-WRF/CESM-delta-SLP-track-no-weak-experiments.txt','wb')
pickle.dump(save,f)
f.close()
