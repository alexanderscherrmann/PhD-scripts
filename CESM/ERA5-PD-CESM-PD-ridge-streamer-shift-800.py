...
simc = ['400-km-west','800-km-west','400-km-east','800-km-east','400-km-north','800-km-north','400-km-south','800-km-south','-0-km']
ticklabels=['4W','8W','4E','8E','4N','8N','4S','8S','C']
...
labels2= ['E2010 moderate','C2010 moderate','C2040','C2070','C2100']
amp='1.4'
labels=['(a)','(b)','(c)','(d)','(e)']

for samp,wamp,qamp in zip(['weak','moderate','strong'],[11,22,34],['0.7','1.4','2.1']):
    fig=plt.figure(figsize=(8,8))
    labels2=['E2010 %s'%samp,'C2010 %s'%samp]
    amp=qamp
    for q,perio in enumerate(period[:2]):
     ...
     for simid,sim in enumerate(SIMS):
        if not amp in sim or not perio in sim or '200' in sim:
            continue
    ...
    fig.savefig(pappath + 'ERA5-PD-CESM-PD-ridge-shift-800km-%s.png'%samp,dpi=300,bbox_inches='tight')
    plt.close('all')
