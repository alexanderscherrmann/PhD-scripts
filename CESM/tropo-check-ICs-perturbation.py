...
#sims=['CESM-2100-DJF-2.1-10km-300K','CESM-2100-DJF-2.1-15km-300K','CESM-2100-DJF-2.1-16km-300K','CESM-2100-DJF-2.1-10km-280K','CESM-2100-DJF-2.1-13km-290K']
sims=['DJF-clim-max-U-at-300-hPa-2.1-QGPV','DJF-clim-max-U-at-300-hPa-1.4-QGPV','DJF-clim-max-U-at-300-hPa-0.7-QGPV','MAM-clim-max-U-at-300-hPa-2.1-QGPV','SON-clim-max-U-at-300-hPa-2.1-QGPV']
...
# for q,perio in enumerate(period):
  for q,sim in enumerate(sims):
    ...
    ref = ds(wrfd + 'CESM-2100-DJF-clim/wrfout_d01_2000-12-01_00:00:00','r')
    Uref = ref.variables['U'][0,:]
    Pref = wrf.getvar(ref,'pressure')
#    U300ref = wrf.interplevel((Uref[:,:,:-1] + Uref[:,:,1:])/2,Pref,300,meta=False)
    U300ref = (Uref[:,:,:-1] + Uref[:,:,1:])/2


    per = ds(wrfd + '%s/wrfout_d01_2000-12-01_00:00:00'%sim,'r')
    ...
    ght = wrf.getvar(ref,'geopotential')/9.81

    MSLPper = wrf.getvar(per,'slp')
...
#    U300per = wrf.interplevel((Uper[:,:,:-1] + Uper[:,:,1:])/2,Pper,300,meta=False)
    ...
    zmax,ymax,xmax = np.where(U300ref[:32,:100,:170]==np.max(U300ref[:32,:80,:150]))
    print(zmax,ymax,xmax,ght[zmax,ymax,xmax])
...
#    ymax=ymax+y0
    ...
    nax.set_aspect('auto')
    text = nax.text(xtext,ytext,sim,transform=nax.transAxes,zorder=100)
    ...
    text = nax4.text(xtext,ytext,sim,transform=nax4.transAxes,zorder=100)
    ### vertical U ref
...
Fig.savefig(pappath + 'CESM-tropo-ICs-PV-TH-check.png',dpi=300,bbox_inches='tight')
plt.close('all')
