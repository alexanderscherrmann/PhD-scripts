...
inp="/net/thermo/atmosdyn2/ascherrmann/017-CESM-WRF-2/"
memb="0900"
out=${inp}
...
do
	...
	let "yy = ${y}+1"	
	echo $yy
	ncks -d time,${y}-12-01T00:00:00,${yy}-01-01T00:00:00,2 -d time,${y}-01-01T00:00:00,${y}-02-28T23:00:00,2 *h3.${y}-01-01-00000.nc time-sel-${y}.nc

	rm *h3.${y}-01-01-00000.nc

#	# calculate P use it for RH and set RH everywhere to 100 when RH>100
	ncap2 -t 2 -s 'P[time,lev,lat,lon]=(hyam*1000 *100+ hybm*PS); RH=(0.263 * P * Q/(exp(17.67 * (T-273.16)/(T-29.65)))); where(RH>100) RH=100;' time-sel-${y}.nc RH-${y}.nc	

	rm time-sel-${y}.nc
        #interpolate to pressure levels defined above in vrt_prs.nc
	ncremap -t 2 --vrt_fl=vrt_prs.nc --vrt_xtr=nrs_ngh --vrt_ntp=log -v U,V,T,RH,Z3 RH-${y}.nc interpolated-${y}.nc

	# get seasonal and monthly sums
	cdo timsum interpolated-${y}.nc sum-${y}.nc
	ncks -d time,${y}-12-01T00:00:00,${yy}-01-01T00:00:00 interpolated-${y}.nc DEC-${y}.nc
        ncks -d time,${y}-01-01T00:00:00,${y}-01-31T23:00:00 interpolated-${y}.nc JAN-${y}.nc
	ncks -d time,${y}-02-01T00:00:00,${y}-02-28T23:00:00 interpolated-${y}.nc FEB-${y}.nc
...
EOF
	...
	cdo timsum DEC-${y}.nc sum-DEC-${y}.nc
	cdo timsum JAN-${y}.nc sum-JAN-${y}.nc
	cdo timsum FEB-${y}.nc sum-FEB-${y}.nc
