...
for q,l in enumerate(k):
    ...
    cn=ax.contourf(lon,lat,d[l[:5] + '2010' + l[9:]]['total']-d[l]['total'],levels=np.arange(-50,51,10),cmap=cmap,extend='both')
    if q==0:
        av=d[l[:5] + '2010' + l[9:]]['total']-d[l]['total']
    else:
        av+=d[l[:5] + '2010' + l[9:]]['total']-d[l]['total']
...
fig.savefig(pappath + 'av-precipitation-diff-PD.png',dpi=300,bbox_inches='tight')
plt.close('all')
...
cn=ax.contourf(lon,lat,av-(d[l[:5] + '2010' + l[9:]]['total']-d[l]['total']),levels=np.arange(-50,51,10),cmap=cmap,extend='both')
pos = ax.get_position()
cbax = fig.add_axes([pos.x0+pos.width,pos.y0,0.02,pos.height])
cbar=plt.colorbar(cn,cax=cbax,ticks=np.arange(-50,51,10))
cbax.set_yticklabels(np.arange(-50,51,10))
cbax.set_ylabel('[mm (9 d)$^{-1}$]')
fig.savefig(pappath + 'av-precipitation-diff-minus-climatology-diff-PD.png',dpi=300,bbox_inches='tight')
plt.close('all')
