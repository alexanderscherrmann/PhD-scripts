...
precip = (Pdata[:,-2]*1000 + Pdata[:,-1]*1000).reshape(-1,49)
TH = Pdata[:,-4].reshape(-1,49)
THE = Pdata[:,-3].reshape(-1,49)
...
for ll,k in enumerate(datadi.keys()):
    ...
    if k=='119896':# or k=='540325':
        t = datadi[d]['time'][0]
        oroids = np.append(oroids,k)
        fig, axes = plt.subplots(4,1,sharex=True)
        axes = axes.flatten()
        qqq = len(i)
        for qq, ax in enumerate(axes[:1]):
        ...
        axes[0].set_ylabel('topo. PV [PVU]')
        axes[1].set_ylabel('dPSP [hPa]')

        axes[1].legend(split,fontsize=fsl,loc='upper left')
        i2 = np.where((PV>=0.75) & (ORO[k]['env'][:,0]/PV>=0.5))[0]
        for iq in i2:
            axes[2].plot(t,TH[iq,:],color='grey',linewidth=0.1)

        axes[2].set_ylabel('TH [K]')
#        axes[2].set_ylim(1015,600)

        axes[3].set_ylabel('THE [K]')
        for iq in i2:
            axes[1].plot(t,datadi[d]['PS'][iq,:]-datadi[d]['P'][iq,:],color='grey',linewidth=0.05)
            axes[3].plot(t,THE[iq,:],color='grey',linewidth=0.05)
