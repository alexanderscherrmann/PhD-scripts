...
globalcounts = np.zeros(len(radii)+1)
...
#            if ( np.sqrt( (CLON[e]-datadi[date]['lon'][tr,e])**2 + (CLAT[e]-datadi[date]['lat'][tr,e])**2) <=  deltaLONLAT):
    ...
    for key in split:
        ...
        if wql==0:
           ...
           meancounts[key] = np.sum(dit[date][key][idp,:])
        else:
           meandi[key] = np.append(meandi[key], dipv[date][key][idp,0])
           meancounts[key] += np.sum(dit[date][key][idp,:])
    wql=10

  globalpercentage[idee+1] = np.mean(meandi[cyc])
  globalcounts[idee+1] = meancounts[cyc]/(meancounts[cyc]+meancounts[key])

globalpercentage[0] = 0
globalcounts[0] = globalcounts[1]

fig, axes = plt.subplots()
axes.plot(np.append(0,radii),globalpercentage)
axes.set_ylabel('average PV modification inside the cyclone [PVU]')
...
#fig, axes = plt.subplots()
#axes.plot(np.append(0,radii),globalcounts*100)
#axes.set_ylabel('relative cyclonic modification counts [%]')
#axes.set_xlabel('cyclone effective radius [km]')
#axes.set_xticks(ticks=np.arange(0,1100,200))
#
#axes.axvline(400,color='grey',ls='-')
#axes.set_xlim(0,1000)
##axes.set_ylim(0,40)
#name = 'cyclonic-counts-percentage-400-0hours.png'
#fig.savefig(psave + name,dpi=300,bbox_inches="tight")
#plt.close('all')
