...
#    lonc = np.mean(np.unique(tralon[:,0]))
    ...
    minid = np.where(ldata[mon][CYID]['hzeta']==0)[0][0]
    ax.scatter(tracklo[minid],trackla[minid],color='k',marker='o',s=25,zorder=50)
    circ2 = patch.Circle((tracklo[minid],trackla[minid]),helper.convert_radial_distance_to_lon_lat_dis(200),edgecolor='k',facecolor='none',linestyle='--',linewidth=1.5,zorder=10.)
    circ1 = patch.Circle((tracklo[minid],trackla[minid]),helper.convert_radial_distance_to_lon_lat_dis(400),edgecolor='k',linestyle='-',facecolor='None',linewidth=1.5,zorder=10.)
...
#    ax.text(-0.07, 0.95, te, transform=ax.transAxes,fontsize=14, va='top')
    figname = '/atmosdyn2/ascherrmann/paper/cyc-env-PV/review/traj-pvr-' + str(date) + '.png'
#    fig.savefig('/atmosdyn2/ascherrmann/paper/cyc-env-PV/test123.png')
#    print(figname) 
